<section>
    <p class="text-sm text-warm-500 mb-4">Update your account's profile information and email address.</p>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="space-y-4">
        ...
        <div>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-warm-600">
                        {{ __('Your email address is unverified.') }}
                        <button form="send-verification" class="underline text-sm text-mental-600 hover:text-mental-700 transition-colors">
