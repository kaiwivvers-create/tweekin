<footer class="border-t border-warm-200 bg-white/50 mt-16">
    <div class="w-full px-6 sm:px-8 lg:px-12 py-10">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-8">
            ...
            <div>
                <div class="flex items-center gap-2.5 mb-3">
                    ...
                    @endif
                    <span class="font-display font-bold text-base text-warm-800">{{ $brand['name'] }}</span>
                </div>
            ...
            <div>
                ...
                <div class="flex gap-3">
                    @if($brand['social_instagram'] ?? '')
                        <a href="{{ $brand['social_instagram'] }}" target="_blank" rel="noopener" class="w-9 h-9 rounded-lg bg-warm-100 hover:bg-warm-200 flex items-center justify-center transition-colors" aria-label="Instagram">
                    ...
                    @endif
                    @if($brand['social_twitter'] ?? '')
                        <a href="{{ $brand['social_twitter'] }}" target="_blank" rel="noopener" class="w-9 h-9 rounded-lg bg-warm-100 hover:bg-warm-200 flex items-center justify-center transition-colors" aria-label="Twitter / X">
                    ...
                    @endif
                    @if($brand['social_tiktok'] ?? '')
                        <a href="{{ $brand['social_tiktok'] }}" target="_blank" rel="noopener" class="w-9 h-9 rounded-lg bg-warm-100 hover:bg-warm-200 flex items-center justify-center transition-colors" aria-label="TikTok">
                    ...
                    @endif
                    @if($brand['social_github'] ?? '')
                        <a href="{{ $brand['social_github'] }}" target="_blank" rel="noopener" class="w-9 h-9 rounded-lg bg-warm-100 hover:bg-warm-200 flex items-center justify-center transition-colors" aria-label="GitHub">
                    ...
                    {{-- Show placeholder if no socials configured --}}
                    @unless($brand['social_instagram'] ?? '' || $brand['social_twitter'] ?? '' || $brand['social_tiktok'] ?? '' || $brand['social_github'] ?? '')
                        <span class="text-xs text-warm-400 italic">No social links configured yet.</span>
                    @endunless
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-warm-200 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-warm-400">
            <span>&copy; {{ date('Y') }} {{ $brand['name'] }}. For informational purposes only.</span>
